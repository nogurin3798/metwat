const CACHE='metwat-pwa-v48-autonow-1';
...
self.addEventListener('fetch',e=>{ e.respondWith(caches.match(e.request).then(c=> c || fetch(e.request).then(r=>{caches.open(CACHE).then(C=>C.put(e.request,r.clone())); return r;}))); });