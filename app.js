...
function LineChart({ data, xKey, yKey, label }){
  ...
  const circles = pts.map((p,i)=> React.createElement('circle',{key:i,cx:sx(p.x), cy:sy(p.y), r:3, fill:'#0ea5e9'}));
  ...
  return React.createElement('div',{className:'container'},
    ...
    React.createElement('div',{className:'card pad', style:{marginTop:16}},
      ...
      React.createElement('div',{className:'grid', style:{gridTemplateColumns:'1fr', marginTop:8}},
        React.createElement(LineChart,{data:[...log].reverse(), xKey:'dt', yKey:'avgHR', label:'平均心拍 (bpm) 推移'}),
        React.createElement(LineChart,{data:[...log].reverse(), xKey:'dt', yKey:'MET', label:'MET 推移'}),
