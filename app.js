...
function App() {
  ...
  const [autoNow, setAutoNow] = useState(
    () => (localStorage.getItem('rehab_auto_now') ?? '1') === '1'
  );
  useEffect(() => {
    localStorage.setItem('rehab_auto_now', autoNow ? '1' : '0');
  }, [autoNow]);
  ...
  const [bpPreSys, setBpPreSys] = useState('');
  const [bpPreDia, setBpPreDia] = useState('');
  const [bpPostSys, setBpPostSys] = useState('');
  const [bpPostDia, setBpPostDia] = useState('');

  const [bp5Sys, setBp5Sys] = useState('112');
  const [bp5Dia, setBp5Dia] = useState('79');

  // 外部 kcal 入力（ウォッチ/YAMAP/手入力）
  ...
  const [weekStart, setWeekStart] = useState(() => {
    try {
      return localStorage.getItem('rehab_week_start') || 'sun';
    } catch (_) {
      return 'sun';
    }
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      setRestHR(v.restHR ?? '');
      setAvgHR(v.avgHR ?? '');
      setHrAfter(v.hrAfter ?? '');
      setHr5min(v.hr5min ?? '');
      setBorgPre(v.borgPre ?? '');
      setBorgPost(v.borgPost ?? '');
      setBpPreSys(v.bpPreSys ?? '');
      setBpPreDia(v.bpPreDia ?? '');
      setBpPostSys(v.bpPostSys ?? '');
      setBpPostDia(v.bpPostDia ?? '');
      setBp5Sys(v.bp5Sys ?? '');
      setBp5Dia(v.bp5Dia ?? '');
      setExtKcal(v.extKcal ?? '');
      setExtSrc(v.extSrc ?? 'watch');
      setPreferExt(v.preferExt ?? true);
    } catch {}
  }, []);

  // 自動計算（速度v[m/min], 勾配, VO2, MET, kcal, WAT, METs・時）
  const auto = useMemo(() => {
  ...
  // 外部kcalから逆算（MET, VO2, WAT, METs・時）
  const ext = useMemo(() => {
    const kcal = num(extKcal), w = num(weight), t = num(time);
    const time_hr = Number.isFinite(t) ? t / 60 : NaN;
    const MET =
      Number.isFinite(kcal) && Number.isFinite(w) && Number.isFinite(time_hr) && w > 0 && time_hr > 0
        ? kcal / (w * time_hr)
        : NaN;
  ...
  const HRcalc = useMemo(() => {
    ...
    const low = Number.isFinite(HRR) && Number.isFinite(R) ? R + 0.4 * HRR : NaN;
    const high = Number.isFinite(HRR) && Number.isFinite(R) ? R + 0.6 * HRR : NaN;
  ...
  const [log, setLog] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('rehab_v48_log') || localStorage.getItem('rehab_v47_log') || '[]');
    } catch {
      return [];
    }
  });
  ...
  const saveLog = () => {
    ...
    const entry = {
      ...
      bp5Sys,
      bp5Dia,
      auto_MET: Number(auto.MET?.toFixed(2)),
      auto_VO2: Number(auto.VO2?.toFixed(1)),
      auto_kcal: Number(auto.kcal?.toFixed(0)),
      auto_WAT: Number(auto.WAT?.toFixed(0)),
      auto_METh: Number(auto.MET_h?.toFixed(2)),
      ext_MET: Number(ext.MET?.toFixed(2)),
      ext_VO2: Number(ext.VO2?.toFixed(1)),
      ext_kcal: Number(ext.kcal?.toFixed(0)),
      ext_WAT: Number(ext.WAT?.toFixed(0)),
      ext_METh: Number(ext.MET_h?.toFixed(2)),
      used_METh: Number((preferExt ? ext.MET_h : auto.MET_h)?.toFixed(2)),
      speed_kmh: Number(auto.kmh?.toFixed(2)),
      grade_pct: Number((auto.grade * 100)?.toFixed(1)),
      extSrc,
      preferExt
    };
  ...
  function SimpleChart({ data, label }) {
    ...
    if (data.length === 0) return React.createElement('div', null, 'まだデータがありません');
  ...
  const MainView = React.createElement(
    ...
    React.createElement(
      'div',
      { className: 'grid inputs', style: { marginTop: 8 } },
      React.createElement(
        'div',
        null,
        React.createElement('label', null, '日時'),
        React.createElement('input', { type: 'datetime-local', value: dt, onChange: e => setDt(e.target.value) }),
        React.createElement(
          'div',
          { className: 'pill', style: { marginTop: 6 } },
          React.createElement('input', { type: 'checkbox', checked: autoNow, onChange: e => setAutoNow(e.target.checked) }),
          React.createElement('span', null, '保存時に現在時刻を自動セット')
        )
      ),
      ...
      React.createElement(
        'div',
        null,
        React.createElement('label', null, '週の開始曜日'),
        React.createElement(
          'select',
    ...
    React.createElement(
      ...
      React.createElement(
        'div',
        null,
        React.createElement('label', null, '外部ソース'),
        React.createElement(
          'select',
          { value: extSrc, onChange: e => setExtSrc(e.target.value) },
      ...
      React.createElement(
        'div',
        null,
        React.createElement('label', null, '表示優先'),
        React.createElement(
          'select',
          { value: preferExt ? 'ext' : 'auto', onChange: e => setPreferExt(e.target.value === 'ext') },
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          'div',
          { className: weekly.sum >= 23 ? 'ok' : 'warn', style: { marginTop: 6 } },
  ...
  const LogView = React.createElement(
    'div',
    { className: 'card' },
    ...
    React.createElement(
      'div',
      { className: 'btns', style: { marginTop: 8 } },
      React.createElement(
        'button',
        {
          className: 'secondary',
        ...
        'button',
        {
          ...
          onClick: () => {
            if (confirm('すべてのログを削除しますか？')) {
              localStorage.removeItem('rehab_v48_log');
              setLog([]);
            }
          }
        },
  ...
  const ChartsView = React.createElement(
    'div',
    { className: 'card' },
    React.createElement('div', { className: 'title' }, 'グラフ（直近20件）'),
    React.createElement(SimpleChart, { data: dataHR, label: '平均HR (bpm)' }),
    React.createElement('div', { style: { height: 8 } }),
    React.createElement(SimpleChart, { data: dataMET, label: 'MET' }),
    React.createElement('div', { style: { height: 8 } }),
    React.createElement(SimpleChart, { data: dataWAT, label: 'WAT (W)' })
  );

  const HelpView = React.createElement(
    'div',
    { className: 'card' },
    React.createElement('div', { className: 'title' }, '解説（数式・METs・時・週開始・自動時刻）'),
    React.createElement(
      'div',
      { className: 'code' },
      `歩行VO2 = 3.5 + 0.1×v + 1.8×v×勾配
MET = VO2 / 3.5
消費エネルギー(kcal) = MET × 時間[h] × 体重[kg]
外部kcalからの逆算: MET = kcal / (体重 × 時間[h])`
    ),
    React.createElement('h3', null, '自動時刻'),
    React.createElement('p', null, '「保存時に現在時刻を自動セット」がONだと、保存ボタンを押した瞬間の時刻をログに記録し、入力欄の日時も更新します（設定は端末に保存）。'),
    React.createElement('h3', null, '週間目標（参考：厚労省）'),
    React.createElement('p', null, '最低 23 METs・時 / 週、推奨 33 METs・時 / 週 を目安にバーで可視化しています。')
  );
